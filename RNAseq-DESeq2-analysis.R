...
## Requires a sample_table.csv, Annotation.csv and a feature table.csv
...
## Set wd containing sample_table.csv
indir = (".")
csvfile <- file.path(indir, "sample_table.csv")
sampleTable <- read.csv(csvfile)

readcounts <- read.csv(file="featuretable.csv", header = TRUE, row.names = 1)
...
annot <- read.csv(annotation, header = TRUE)
...
pheatmap(mat, annotation_col=df, fontsize_row = 4)
dev.off()
